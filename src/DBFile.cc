...
DBFile::DBFile (File &otherFile, Input &otherRFile, Settings &otherConfig, ComparisonEngine &otherComp):
...
int DBFile::Create (const char *f_path, fType f_type, void *startup) {
	...
	if(f_path == NULL) success = false;
		...
		if(f_path == NULL ||
			...
			if(!success) {
				file.Close();
				if(rawOpen) rfile.Close(); // Closing an unopened Input segfaults
				remove(f_path);
				remove(header);
				Reset();
			}
