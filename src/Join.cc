...
void Join::Work(Pipe &inPipeL, Pipe &inPipeR, Pipe &outPipe, CNF &selOp, Record &literal,
	...
	else LargeJoin(inPipeL, inPipeR, outPipe, selOp, literal);
}
...
void Join::SortMergeJoin(Pipe &pipeL, Pipe &pipeR, Pipe &outPipe, OrderMaker &orderL,
		OrderMaker &orderR, CreateJoin &relR, CreateJoin &relS, Record &tempL, Record &tempR) {

}

void Join::LargeJoin(Pipe &pipeL, Pipe &pipeR, Pipe &outPipe, CNF &selOp, Record &literal) {
	CreateJoin relS = CreateJoin(memLimit);
	FindRelation relR;
	Record rec;
	LargeJoin(pipeL, pipeR, outPipe, selOp, literal, relS, relR, rec);
}

void Join::LargeJoin(Pipe &pipeL, Pipe &pipeR, Pipe &outPipe, CNF &selOp, Record &literal,
		CreateJoin &relS, FindRelation &relR, Record &rec) {
	Record *temp = &rec;
	ComparisonEngine comp;
	...
	if(relS.MemUsed() > 0) { // There is work to be done
		...
		while(pipeL.Remove(temp)) {
			if(!relR.Add(temp)) {
				CombineRelations(relR, relS, rec, outPipe, comp, literal, selOp);
		...
		if(relR.MemUsed() > 0) CombineRelations(relR, relS, rec, outPipe, comp, literal, selOp);
	}
	outPipe.ShutDown();
}

void Join::CombineRelations(Relation &R, Relation &S, Record &rec, Pipe &out) {

}

void Join::CombineRelations(Relation &R, Relation &S, Record &rec, Pipe &out, ComparisonEngine &comp, Record &literal, CNF &cnf) {
