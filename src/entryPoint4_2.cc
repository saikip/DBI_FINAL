...
int main(int argc, char const *argv[]) {
	char* catalogFName = "tpch/catalog";	
	string dbLocation = "tpch/";
	Statistics stats = Statistics();
	stats.Read("stats.txt");
	RelationData relations = RelationData();
	char* rels[] = {"supplier", "nation", "region", "customer", "part", "partsupp", "orders", "lineitem", "mytable"};
	...
	for(int i = 0; i < 8; ++i) {
        ...
        setupRelation(rels[8], catalogFName, dbLocation+string(rels[8])+".bin", relations, stats);
