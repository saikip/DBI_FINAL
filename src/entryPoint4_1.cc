...
void q2 (){
	...
	//if(result!=800000)
	//	cout<<"error in estimating Q2 before apply \n ";
	cout<<" Your estimation Result  " <<result;
...
//	s.Read(fileName);
	...
	if(fabs(result-21432.9)>0.5)
		cout<<"error in estimating Q12\n";
	cout<<" Your estimation Result  " <<result;
	cout<<"\n Correct Answer: 21432";
...
int main(int argc, char *argv[]) {
	if (argc < 2) {
		cerr << "\n\n You need to supply me the query number to run as a command-line arg.." << endl;
		cerr << "Usage: ./bin/entryPoint4_1 [1-12] \n\n" << endl;
		exit (1);
	}

	void (*query_ptr[]) () = {&q1, &q2, &q3, &q4, &q5, &q6, &q7, &q8,&q9,&q10,&q11,&q12};  
	void (*query) ();
	...
	if (qindx >=1 && qindx < 13) {
		...
		cout << "\n\n ERROR! Enter a value from 1 to 12. \n\n Corresponding query in file 'testcases'. \n\n";
	}
